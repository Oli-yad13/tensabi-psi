...
export default function EditPharmacyPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    if (pharmacy.hoursJson) setHours(pharmacy.hoursJson as Record<string, string>);
  }, [pharmacy]);
  ...
  if (!pharmacy) return <div className="p-8 text-red-400 text-sm">Pharmacy not found.</div>;
    ...
    <div className="p-8 max-w-2xl">
      <div className="flex items-center justify-between mb-7">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-extrabold text-slate-800">Edit Pharmacy</h1>
            <p className="text-xs text-slate-400 font-medium mt-0.5">{pharmacy.name}</p>
          </div>
        </div>
        {!confirmDelete ? (
