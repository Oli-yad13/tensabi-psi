...
export default function NewTestingCenterPage() {
  ...
  const { mutate: create, isPending } = useMutation({
    ...
    onError: (e: any) => setError(e?.response?.data?.message ?? 'Failed to create center'),
  });
  ...
  return (
    <div className="p-8 max-w-2xl">
      <div className="flex items-center gap-3 mb-7">
        ...
        <div>
          <h1 className="text-xl font-extrabold text-slate-800">Add Testing Center</h1>
          <p className="text-xs text-slate-400 font-medium mt-0.5">Create a new STD/HIV testing location</p>
        </div>
      </div>
      ...
      <div className="space-y-5">
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 space-y-4">
          ...
          <Field label="Center Name *">
            <input className={input} value={form.name} onChange={(e) => set('name', e.target.value)} placeholder="e.g. Addis Ababa Health Center" />
          </Field>

          <Field label="Address *">
            <input className={input} value={form.address} onChange={(e) => set('address', e.target.value)} placeholder="e.g. Bole Sub-City, Woreda 3" />
          </Field>
