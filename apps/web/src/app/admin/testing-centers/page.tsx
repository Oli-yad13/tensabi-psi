...
export default function AdminTestingCentersPage() {
  const [search, setSearch] = useState('');
  const [deleting, setDeleting] = useState<string | null>(null);
  const qc = useQueryClient();

  const { data: centers = [], isLoading } = useQuery<Pharmacy[]>({
    queryKey: ['testing-centers', search],
    queryFn: () => api.get(`/pharmacies?type=TESTING_CENTER&search=${search}`).then((r) => r.data),
  });
  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-extrabold text-slate-800">Testing Centers</h1>
          <p className="text-sm text-slate-500 mt-1">Manage STD & HIV testing locations</p>
        </div>
        <Link
          href="/admin/testing-centers/new"
      ...
      <div className="relative mb-5 max-w-sm">
        ...
        <input
          ...
          placeholder="Search testing centers..."
          value={search}
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-slate-50 text-left border-b border-slate-100">
              <th className="px-6 py-3.5 text-xs font-extrabold text-slate-400 uppercase tracking-wider">Center Name</th>
            ...
            {isLoading ? (
              ...
              <tr>
                <td colSpan={6} className="px-6 py-16 text-center text-slate-400">
                  ...
                  <p className="font-semibold text-sm">No testing centers found</p>
                  <Link href="/admin/testing-centers/new" className="text-xs text-[#037561] font-bold mt-2 inline-block hover:text-purple-800">
                    + Add the first one
                  </Link>
                </td>
              </tr>
              ...
              centers.map((c) => (
                <tr key={c.id} className="hover:bg-slate-50 transition-colors group">
                  ...
                  <td className="px-6 py-4 text-slate-500 text-xs">{c.phone ?? '—'}</td>
                  <td className="px-6 py-4">
                    <span className={`text-xs font-bold px-2.5 py-1 rounded-full ${c.isOpen ? 'bg-green-50 text-green-700' : 'bg-slate-100 text-slate-500'}`}>
                      {c.isOpen ? 'Open' : 'Closed'}
                    </span>
                  </td>
                  <td className="px-6 py-4 text-slate-500 text-xs">
                    {c.testKits?.length ?? 0} kit{(c.testKits?.length ?? 0) !== 1 ? 's' : ''}
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <Link
                        href={`/admin/testing-centers/${c.id}/edit`}
                      ...
                      {deleting === c.id ? (
                        <div className="flex items-center gap-2 text-xs">
                          ...
                          <button onClick={() => remove(c.id)} className="text-red-600 font-bold hover:text-red-800">Yes</button>
        ...
        {!isLoading && centers.length > 0 && (
          <div className="px-6 py-3 border-t border-slate-50 text-xs text-slate-400 font-medium">
            {centers.length} testing center{centers.length !== 1 ? 's' : ''} total
          </div>
        )}
      </div>
    </div>
